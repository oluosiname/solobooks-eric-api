...
/**
 * Enumeration aller Otto-Fehlercodes.
 */
public enum ErrorCode {
     ...
     /** Es wurde keine oder eine ungültige Hersteller-ID angegeben. */
     ...
     /** Die Einstellung ist unbekannt */
     OTTO_EINSTELLUNG_UNBEKANNT(610405023),
     
     /** Der Wert ist für diese Einstellung nicht zulässig */
     OTTO_EINSTELLUNG_WERT_UNGUELTIG(610405024),
     ...
     /** eSigner: Die ASN.1-Struktur enthält kein Enveloped Data. */
     OTTO_ESIGNER_ASN1_NO_ENVELOPED_DATA(610405847),
     
     /** eSigner: Die ASN.1-Struktur enthält keine Daten. */
    ...
    /**
     * Gibt den Fehlercode zu einem Integer-Wert zurück
     *
     * @param errorCode Fehlercode-Nummer
     * @return Errorcode Klasseninstanz
     */
    public static ErrorCode getErrorCode(final int errorCode) {
        for (final ErrorCode curr : ErrorCode.values()) {
            if (curr.value() == errorCode) {
                return curr;
            }
        }

        throw new IllegalArgumentException("Unbekannter Fehlercode: " + errorCode);
    }

    /**
     * Gibt die Integer-Repräsentation des Fehlercodes zurück
     *
